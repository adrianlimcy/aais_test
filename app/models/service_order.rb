class ServiceOrder < ApplicationRecord
  ...
  def initialize(service_order)
    ...
    #response = client.call(:aai_service, message: { AUserID: ENV["AAIS_USER"],
    #                                                AEntity: ENV["AENTITY"],
    #                                                ServiceName: ENV["SERVICENAME"],
    #                                                ParameterList: service_order})
    ...
    if response.success?
      #data = response.to_array(:aai_service_response, :plc_ob_entity).first
      data = response.to_hash[:aai_service_response][:plc_ob_entity]
      test = Nokogiri::XML(data)
      if data
        @service_no = test.xpath("//Service-no").first.children.first.to_s
